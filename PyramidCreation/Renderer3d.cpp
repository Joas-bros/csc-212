...
void Renderer3D::render()
{
	...
	pulsation = sinf(SDL_GetTicks() * 0.001f);
...
{
	float scale = 1.0f + pulsation * 0.5f; 
	return Point2D{ WindowSizeX / 2 + (point.x * FL * scale) / (FL + point.z) * 100,
					 WindowSizeY / 2 + (point.y * FL * scale) / (FL + point.z) * 100 };
}